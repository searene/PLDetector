  ...
  module DataArray
    ...
    def encode_data
      if @data.first.is_a?(Numeric)
        ...
        sets = [@data.collect { |d| GoogleChart::encode_sentence(encoding,d,max_val)}.join(get_data_separator)]
        ...
        x_sets = x_values.collect { |set| set.collect{ |d| GoogleChart::encode_sentence(encoding,d,x_max_val)}.join(get_data_separator) }
        y_sets = y_values.collect { |set| set.collect{ |d| GoogleChart::encode_sentence(encoding,d,y_max_val)}.join(get_data_separator) }
        ...
        sets = @data.collect { |set| set.collect{ |d| GoogleChart::encode_sentence(encoding,d,max_val)}.join(get_data_separator) }
      end
      return sets
    end
